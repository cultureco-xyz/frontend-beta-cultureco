...
export const RazorpayPurchaseButton = ({
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const orderRes = await fetch("/backend/payment/razorpay/order-product", {
        method: "POST",
        body: JSON.stringify({ amount: cost, creator, productId }),
      ...
      const options = {
        ...
        currency: currency,
        name: "CultureCo",
        description: "Test Product Purchase Transaction",
        order_id: order_id,
        handler: async function (response: {
          ...
          await fetch("/backend/payment/razorpay/verify-product", {
            method: "POST",
            body: JSON.stringify({
  ...
  return (
    <Button
      ...
      disabled={isLoading}
    >
      {isLoading ? "Loading..." : "Pay with UPI"}
    </Button>
  );
};
