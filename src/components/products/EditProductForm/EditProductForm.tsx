...
interface EditProductFormProps {
  ...
  productData: {
    type: TProductTabs;
    data: any; // Replace with appropriate type for your product data
  };
}

function EditProductForm({ close, productData }: EditProductFormProps) {
  const [productType, setProductType] = useState<TProductTabs>(
    productData.type || TProductTabs.DIGITAL
  );

  const [initialData, setInitialData] = useState(productData.data || null);
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="flex overflow-y-auto flex-col pt-4 w-full max-w-mobile mb-[56px] max-h-[86%] h-fit rounded-t-xl shadow-2xl bg-cultureGray absolute bottom-0 z-[999] px-4"
      >
        <span
        ...
        {productType == TProductTabs.DIGITAL && (
          <DigitalForm initialData={productData.data} />
        )}
        {productType == TProductTabs.PHYSICAL && (
          <PhysicalForm initialData={productData.data} />
        )}
        {productType == TProductTabs.EVENT && (
          <EventForm initialData={productData.data} />
        )}
      </motion.div>
    </div>
  );
